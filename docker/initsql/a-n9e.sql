...
--     `rule_algo` varchar(255) not null trigger_value` varchar(255) not null,
--     `annotations` text not null comment 'annotations',
...
-- ) ENGINE=InnoDB DEFAULT CHARSET = utf8mb4;
CREATE TABLE `alert_cur_event` (
  `id` bigint(20) unsigned NOT NULL COMMENT 'use alert_his_event.id',
  `cate` varchar(128) NOT NULL,
  `datasource_id` bigint(20) NOT NULL DEFAULT '0' COMMENT 'datasource id',
  `cluster` varchar(128) NOT NULL,
  `group_id` bigint(20) unsigned NOT NULL COMMENT 'busi group id of rule',
  `group_name` varchar(255) NOT NULL DEFAULT '' COMMENT 'busi group name',
  `hash` varchar(64) NOT NULL COMMENT 'rule_id + vector_pk',
  ...
  `severity` tinyint(1) NOT NULL COMMENT '0:Emergency 1:Warning 2:Notice',
  `prom_for_duration` int(11) NOT NULL COMMENT 'prometheus for, unit:s',
  `prom_ql` varchar(8192) NOT NULL COMMENT 'promql',
  `prom_eval_interval` int(11) NOT NULL COMMENT 'evaluate interval',
  `callbacks` varchar(255) NOT NULL DEFAULT '' COMMENT 'split by space: http://a.com/api/x http://a.com/api/y',
  `runbook_url` varchar(255) DEFAULT NULL,
  `notify_recovered` tinyint(1) NOT NULL COMMENT 'whether notify when recovery',
  `notify_channels` varchar(255) NOT NULL DEFAULT '' COMMENT 'split by space: sms voice email dingtalk wecom',
  `notify_groups` varchar(255) NOT NULL DEFAULT '' COMMENT 'split by space: 233 43',
  `notify_repeat_next` bigint(20) NOT NULL DEFAULT '0' COMMENT 'next timestamp to notify, get repeat settings from rule',
  `notify_cur_number` int(11) NOT NULL DEFAULT '0',
  `target_ident` varchar(191) NOT NULL DEFAULT '' COMMENT 'target ident, also in tags',
  `target_note` varchar(191) NOT NULL DEFAULT '' COMMENT 'target note',
  `first_trigger_time` bigint(20) DEFAULT NULL,
  ...
  KEY `hash` (`hash`),
  KEY `rule_id` (`rule_id`),
  KEY `trigger_time` (`trigger_time`,`group_id`),
  KEY `notify_repeat_next` (`notify_repeat_next`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
