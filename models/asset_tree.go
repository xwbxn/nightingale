...
//默认树：设备状态-设备类型-设备厂商
func BuildAssetTree(ctx *ctx.Context, status int64) ([]*FrontTree, error) {
	var tree []*AssetTree
	err := DB(ctx).Select("ID", "NAME", "PARENT_ID, TYPE, PROPERTY_ID").Where("STATUS = ?", status).Find(&tree).Error
	if err != nil {
		return nil, err
	}
	if tree == nil {
