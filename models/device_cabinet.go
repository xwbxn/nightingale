...
type DeviceCabinet struct {
	...
	BelongRoom             int64          `gorm:"column:BELONG_ROOM" json:"belong_room" cn:"所在机房" source:"type=table,table=computer_room,property=id,field=room_name"`                        //type:string     comment:所在机房                                        version:2023-07-11 15:14
	...
	ProducerId             int64          `gorm:"column:PRODUCER_ID" json:"producer_id" cn:"厂商" source:"type=table,table=device_producer,property=id;producer_type,field=alias,val=producer"` //type:string     comment:厂商                                            version:2023-07-11 15:14
	CabinetModel           int64          `gorm:"column:CABINET_MODEL" json:"cabinet_model" cn:"型号" source:"type=table,table=device_model,property=id,field=model"`                           //type:string     comment:型号                                            version:2023-07-11 15:14
	...
	BelongGroup            int64          `gorm:"column:BELONG_GROUP" json:"belong_group" `                                                                                                   //type:*int       comment:所属机柜组                                      version:2023-9-10 16:45
	...
	Usage                  string         `gorm:"column:USAGE" json:"usage" cn:"用途"`                                                                                                          //type:string     comment:用途                                            version:2023-9-10 16:16
	...
	DeviceTotal            int64          `gorm:"-" json:"device_total" swaggerignore:"true"`
}
...
// 按map查询
func DeviceCabinetGetByMap(ctx *ctx.Context, where map[string]interface{}, limit, offset int) ([]DeviceCabinet, error) {
	session := DB(ctx)
	// 分页
	if limit > -1 {
		session = session.Limit(limit).Offset(offset).Order("id")
	...
	var lst []DeviceCabinet
	err := session.Model(&DeviceCabinet{}).Where(where).Find(&lst).Error
	if err != nil {
		return nil, err
	}
...
// 按机柜名称查询
func DeviceCabinetGetByCabinetName(ctx *ctx.Context, cabinetName string) (DeviceCabinet, error) {
	var obj DeviceCabinet
	err := DB(ctx).Where("CABINET_NAME = ?", cabinetName).Find(&obj).Error
	if err != nil {
		return obj, err
	}
...
// 根据传入条件统计个数
func DeviceCabinetCountByMap(ctx *ctx.Context, where map[string]interface{}) (num int64, err error) {
	return Count(DB(ctx).Model(&DeviceCabinet{}).Where(where))
}
