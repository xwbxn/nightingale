...
func UserGroupGetsByUserId(ctx *ctx.Context, userId int64) ([]string, error) {
	var names []string
	err := DB(ctx).Debug().Model(&UserGroupMember{}).Joins("LEFT JOIN user_group ON user_group_member.group_id = user_group.id").
		Group("user_group.name").Where("user_group_member.user_id = ?", userId).Pluck("user_group.name", &names).Error
	return names, err
}
