...
type OperationLog struct {
	...
	ReqMethod   string         `gorm:"column:REQ_METHOD" json:"req_method" `                     //type:string       comment:请求方式      version:2023-11-07 11:46
...
type OperationLogImport struct {
	Id          int64  `gorm:"column:ID;primaryKey" json:"id" cn:"日志编号" xml:"id"`               //type:BIGINT       comment:日志主键      version:2023-10-21 09:14
	Type        string `gorm:"column:TYPE" json:"type" cn:"操作类型" xml:"type"`                    //type:string       comment:类型          version:2023-10-21 09:10
	Object      string `gorm:"column:OBJECT" json:"object" cn:"系统模块" xml:"object"`              //type:string       comment:对象          version:2023-10-21 09:10
	Description string `gorm:"column:DESCRIPTION" json:"description" cn:"描述" xml:"description"` //type:string       comment:描述          version:2023-10-21 09:10
	User        string `gorm:"column:USER" json:"user" cn:"操作人员" xml:"user"`                    //type:string       comment:用户          version:2023-10-21 09:10
	OperTime    string `gorm:"column:OPER_TIME" json:"oper_time" cn:"操作时间" xml:"oper_time"`     //type:*int         comment:执行时间      version:2023-10-21 09:10
}
...
// 按ids查询
func OperationLogGetByIds(ctx *ctx.Context, ids []int64) ([]*OperationLogImport, error) {
	var lst []*OperationLogImport
	err := DB(ctx).Model(&OperationLog{}).Where("ID in ?", ids).Find(&lst).Error
	return lst, err
}
