...
type MachineRoom struct {
	...
	MachineRoomPicture  string  `gorm:"column:MACHINE_ROOM_PICTURE" json:"machine_room_picture" validate:"omitempty"`         //type:string     comment:机房图片        version:2023-07-11 16:11
...
// TableName 表名:machine_room，机房信息。
// 说明:
func (m *MachineRoom) TableName() string {
	return "machine_room"
}

// 条件查询
func MachineRoomGets(ctx *ctx.Context, query string, limit, offset int) ([]MachineRoom, error) {
	session := DB(ctx)
	// 分页
	if limit > -1 {
...
// 按id查询
func MachineRoomGetById(ctx *ctx.Context, id int64) (*MachineRoom, error) {
	var obj *MachineRoom
...
// 查询所有
func MachineRoomGetsAll(ctx *ctx.Context) ([]MachineRoom, error) {
	var lst []MachineRoom
...
// 增加机房信息
func (m *MachineRoom) Add(ctx *ctx.Context) error {
	// 这里写MachineRoom的业务逻辑，通过error返回错误
...
// 删除机房信息
func (m *MachineRoom) Del(ctx *ctx.Context) error {
	// 这里写MachineRoom的业务逻辑，通过error返回错误
...
// 更新机房信息
func (m *MachineRoom) Update(ctx *ctx.Context, updateFrom interface{}, selectField interface{}, selectFields ...interface{}) error {
	// 这里写MachineRoom的业务逻辑，通过error返回错误
...
// 根据条件统计个数
func MachineRoomCount(ctx *ctx.Context, where string, args ...interface{}) (num int64, err error) {
	return Count(DB(ctx).Model(&MachineRoom{}).Where(where, args...))
}
